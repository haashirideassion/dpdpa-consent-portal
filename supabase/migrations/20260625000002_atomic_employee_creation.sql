-- ============================================================
-- Atomic Employee Creation RPC
-- ============================================================
-- ROOT CAUSE: "Add New Employee" relied on (a) a client-side sequence
-- of separate insert calls, PLUS (b) the on_employee_created AFTER
-- INSERT trigger (20260430000002) to populate the single-entry child
-- detail tables. If that trigger is not active on a given database, or
-- any later client-side call fails, the employees row is created but
-- the linked employee_personal_details / employee_contact_details /
-- employee_employment_details / employee_financial_details /
-- employee_govt_ids / employee_emergency_contacts /
-- employee_additional_details / employee_health_info / consent_records
-- rows never exist — leaving Employee Details blank for every field
-- sourced from those tables.
--
-- FIX: a single SECURITY DEFINER function that inserts the employees
-- row AND every single-entry child row in one Postgres transaction.
-- Postgres functions are atomic — if any statement raises, everything
-- the function did (including the employees insert) rolls back, so an
-- employee can never exist with missing child rows. This does not
-- depend on on_employee_created firing at all; ON CONFLICT clauses
-- make it safe whether that trigger is present, absent, or fires
-- concurrently.
-- ============================================================
...
BEGIN
  ...
  -- 2. Personal + contact details — real values from the form.
